...
{
  apt --simulate install dkms libdrm-dev pkg-config 2>&1 |  grep  "^E: " > /dev/null && return 1
  apt --simulate install dkms libdrm-dev pkg-config | grep -v '^Inst\|^Conf'
}
...
{
  # DKMS
  check_dkms || missing_requirement "DKMS"

  # libdrm
  check_libdrm || missing_requirement "libdrm"

  #pkg-config
  check_pkg || missing_requirement "pkg-config"
